...
const setTxpool = async (config: TPSConfig, deployer: KeyringPair) => {
    ...
    // @ts-ignore
    let blockMaxFee = (
        await api.call.transactionPaymentApi.queryWeightToFee(blockWeight)
    ).toBigInt();
    ...
    for (let k = 0; k < sendersMap.size; k++) {
        ...
        txHash = (
            await api.tx.balances
                .transferKeepAlive(sender.address, amount.toString())
                .signAndSend(deployer, { nonce })
        ).toString();
        if (!validTxHash(txHash))
            throw Error(`[ERROR] batchSendNativeToken() -> ${JSON.stringify(txHash)}`);
        console.log(
            `[batchSendNativeToken] Sending Native Token to ${sender.address} -> ${txHash}`
        );
...
const blockTracker = async (config: TPSConfig) => {
    ...
    const unsubscribe = await api.rpc.chain.subscribeNewHeads(async (header) => {
        if (blockHash != header.hash.toString()) {
            ...
            // @ts-ignore
            let weight = (
                await (await api.at(header.hash)).query.system.blockWeight()
            ).normal.refTime.toNumber();
...
const assertTokenBalances = async (config: TPSConfig) => {
    ...
    for (let k = 0; k < config.accounts; k++) {
        ...
        if (config.txn.module === 'assets') {
            ...
            let {
                data: { free },
            } = await api.query.system.account(receiver.address);
            amount = free;
        }
...
const setupAssets = async (config: TPSConfig, deployer: KeyringPair) => {
    ...
    // @ts-ignore
    console.log(
        `[setupAssets] ${sender.address} Asset [id:${assetId}] balance: ${
            data.isEmpty ? 0 : data.toJSON().balance
        }`
    );
    if (data.isEmpty) {
        let nonce = await api.rpc.system.accountNextIndex(deployer.address);
        for (let k = 0; k < sendersMap.size; k++) {
            ...
            txHash = (
                await api.tx.assets
                    .mint(assetId, sender.address, amount.toString())
                    .signAndSend(deployer, { nonce })
            ).toString();
            if (!validTxHash(txHash))
                throw Error(`[ERROR] setupAssets() -> ${JSON.stringify(txHash)}`);
            console.log(
                `[setupAssets] Minting Asset [id:${assetId}] to ${sender.address} -> ${txHash}`
            );
            if ((k + 1) % 500 === 0) await new Promise((r) => setTimeout(r, 6000));
            // @ts-ignore
            nonce = nonce.add(new BN(1));
...
const setupNFTs = async (config: TPSConfig, deployer: KeyringPair) => {
    ...
    const data = await (
        await api.at(hash)
    ).query.nfts.account(sender.address, collectionId, itemId);
    // @ts-ignore
    console.log(
        `[setupNFTs] ${sender.address} NFT Collection [id:${collectionId}] items: ${
            data.isEmpty ? 'NotOwner' : 'Owner'
        }`
    );
    if (data.isEmpty) {
        let nonce = await api.rpc.system.accountNextIndex(deployer.address);
        ...
        for (let k = 0; k < config.txn.quantity; k++) {
            txHash = (
                await api.tx.nfts
                    .mint(collectionId, itemId, sender.address, {})
                    .signAndSend(deployer, { nonce })
            ).toString();
            if (!validTxHash(txHash))
                throw Error(`[ERROR] setupNFTs() -> ${JSON.stringify(txHash)}`);
            console.log(
                `[setupNFTs] Minting NFT Items [id:${collectionId}] to ${sender.address} -> ${txHash}`
            );
...
const calculateTPS = async (config: TPSConfig, startingBlock: SimpleBlock) => {
    ...
    while (
        lastBlock.extrinsics.length > inherentExtrinsics ||
        lastBlock.number === startingBlock.number
    ) {
    ...
    return `blocks=${blocks} (${startingBlock.number + 1} -> ${lastBlock.number}) | txns=${
        config.txn.quantity
    } t=${t} -> ${config.txn.quantity / t} TPS/RPS ${err}`;
};
